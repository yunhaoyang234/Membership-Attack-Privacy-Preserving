...
class MultiRoomEnvN2(MultiRoomEnv):
    def __init__(self, seed_low, seed_high, **kwargs):
        self.seeds = list(range(seed_low, seed_high))
        # print(seeds)
        super().__init__(
            minNumRooms=2,
            maxNumRooms=2, # 2, 4
    ...
    def reset(self, **kwargs):
        s = self.seeds[np.random.randint(0,len(self.seeds))]
        return super().reset(seed=s)
